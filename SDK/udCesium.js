...
class udCesium {
  constructor(viewer) {
    ...
    this.viewer = viewer;
    this.hasEuleeVtxfPrimitive = 0 //已经初始化EuleeVtxfPrimitive 对象的标记
  ...
  flytoUdsModel(handle) {
    ...
    let extentRadius = new Cesium.Cartesian3(
      headerData.boundingBoxExtents[0],
      headerData.boundingBoxExtents[1],
      headerData.boundingBoxExtents[2]
    );
    let radius = Cesium.Cartesian3.magnitude(extentRadius);
    this.viewer.camera.flyToBoundingSphere(
      new Cesium.BoundingSphere(
        Cesium.Cartesian3.fromDegrees(
          headerData.latLong[1],
          headerData.latLong[0],
          0
        ),
        radius * headerData.scaledRange
      )
    );
  }
  ...
  loadUds(udsModel, cameraPosition) {
    if (Cesium) {
      if (!this.hasEuleeVtxfPrimitive) {
        //console.log("loadUds......");
        this.hasEuleeVtxfPrimitive = 1;
    ...
    if (Cesium) {
      ...
      if (!this.hasEuleeVtxfPrimitive) {
        ...
        for (let i = 0; i < list.length; i++) {
          let handlePair = await loadUDSModel(list[i]);
...
class EuleeVtxfPrimitive {
  constructor(viewer) {
    ...
    function createCommand(context) {
      let _self = this;
      var pickId = this.pickId; //Cesium.PickId
      if (!pickId) {
  ...
  setResolution = (viewer, resolution = 1.0) => {
    // Control Resolution
    let thisWidth = parseInt(viewer.canvas.width * resolution);
    let thisHeight = parseInt(viewer.canvas.height * resolution);
...
const loadUDSModel = async (model) => {
  ...
  let handle2 = udSDKJS_RenderQueueAddModel(
    handle1,
    model.eval[0],
    model.eval[1]
  );
